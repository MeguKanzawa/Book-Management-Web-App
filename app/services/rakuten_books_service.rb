...
def self.search_parent_series(query)
  ...
  def self.fetch_series_volumes(series_name, genre_id:, author: nil, publisher: nil)
...
#   end
  ...
  #     Rails.logger.error "🛑 Rakuten API Error: Credentials missing!"
  #     return []
  #   end
  ...
  #       group_key = "#{normalized_author}_#{normalized_publisher}_#{genre_id}_#{normalized_series}"
        
  #       image_url = item["largeImageUrl"] || item["mediumImageUrl"]
  #       image_url = nil if image_url&.include?("nowprinting")

  #       if series_map[group_key]
  #         # Keep logic for finding the "cleanest" title
